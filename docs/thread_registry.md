...
| 项目 | 当前记录 |
|---|---|
| 当前主线 | 治理架构收口，17 个 proposal 决策记录已提交为 `09de668`，18 个临时 proposal 已列入 30 天删除候选，`ai-intel/raw/*`、`Remod开发.md` 和 PRUNE-01 候选能力收口已完成候选处置记录。 |
| 为什么做 | 用户目标是长期稳定、可监督、可迭代、自我收敛的架构系统；skill / harness / plugin 只是工具。 |
| 当前做到哪 | A/B/D/E 稳定核心和候选能力已分批收口；C 批已完成 demo、fitness、graduation、Jiaxiaoqian、taxi-hailing、AI collaboration、prompt workbench、Santoip、temp-generated 审查；C11 已提交为 `852a2d5 Add deidentified PRD structure golden case`；17 个 proposal 决策记录已提交为 `09de668 Record governance lifecycle decision proposals`；剩余 18 个临时 proposal 已列入 30 天候选清单；`ai-intel/raw/2026-04-30/` 已补齐 30 天删除候选精确清单；`Remod开发.md` 已复核为 30 天删除候选；PRUNE-01 已生成候选能力瘦身复盘报告，并完成 `memory-learning-extractor` 与 delivery suite wording 最小收口；Codex 开发文档真实输出复核已完成，内部默认开发文档模板已收敛为轻量默认 + 完整治理附录，普通 PRD 轻量样例已通过临时目录复核。 |
| 下一步最小动作 | 准备本轮治理总验收报告：汇总已修复项、剩余待处理项和仍需用户拍板项。 |
| 当前不做 | 不转 stable、不新增 skill / harness、不删除、不归档、不提交项目产物、不写长期记忆、不 push / PR。 |
| 最近检查 | 2026-05-04 `git diff --check`、regression、harness check-only 通过；后续每次改文档后继续跑 check-only。 |
...
| ARCHIVE-01 | AI raw 生命周期 | `done` | raw 不应提交，但需要复核和删除候选边界。 | 已提交 raw 30 天删除候选清单；2026-06-02 后二次审批。 | 2026-06-02 后是否删除 raw 本体。 |
| ARCHIVE-02 | `Remod开发.md` | `done` | 旧治理笔记可能已有内容被吸收，也可能误导新增 skill / harness。 | 已复核为 30 天删除候选；不提交原文，不提炼重复总结。 | 2026-06-03 后是否删除原文。 |
| PRUNE-01 | skill / harness / plugin 瘦身 | `done` | 防止工具层膨胀，保持架构轻量。 | 已生成 `candidate_capability_pruning_review.md`，并完成最小收口。 | 后续是否提交报告和相关补丁。 |
...
| E 批 root/archive | root cleanup 已提交最小 archive 证据；`Remod开发.md` 已完成候选处置。 | `答辩.md` 随 `graduation-defense-agent` 项目 closeout 处理。 |
...
| 生命周期嵌入 | `governance_lifecycle_embedding_plan.md` 已生成，并已用于 AI raw、archive notes 和项目产物候选处置。 | 下一步用于 skill / harness / plugin candidate 瘦身复盘。 |
...
| proposal-lifecycle-cleanup | 长期治理 | governance | `active` | 2026-05-03 | 17 个 proposal 决策记录已提交；剩余 18 个未跟踪 proposal 分成 16 个临时材料和 2 个已覆盖旧审查，并已补齐 30 天删除候选精确清单。 | 删除仍需 2026-06-02 后二次批准。 | 提交候选清单；之后转入 raw / archive notes / candidate 瘦身。 |
| thread-lifecycle-supervision | 长期治理 | governance | `active` | 2026-05-02 | 建立线程台账和 5 天停滞总结机制。 | 是否后续创建自动化提醒尚未批准。 | 先用本台账试运行，不创建自动化。 |
| ai-intel-raw-disposition | AI 情报 | governance | `done` | 2026-05-03 | 明确 raw、daily、events、logs、decision docs 的证据边界；raw 只保留为本地复核证据。 | 删除仍需 2026-06-02 后二次批准。 | 等 2026-06-02 后再拿精确清单二次确认。 |
| archive-notes-disposition | 归档治理 | governance | `done` | 2026-05-04 | 处理 `docs/archive/notes/` 和额外 archive 文件，不扩大 E 批。 | `答辩.md` 已在 C9 中识别为 `graduation-defense-agent` 原始输入证据候选；`Remod开发.md` 已确认大部分经验被吸收。 | `Remod开发.md` 等 2026-06-03 后二次审批；`答辩.md` 等项目正式 closeout 时拍板。 |
...
| skill-harness-candidate-pruning | 能力治理 | governance | `done` | 2026-05-04 | 复核 skill / harness / plugin candidate 是否仍必要。 | 已完成只读索引、中文报告、`memory-learning-extractor` 状态收口和 delivery wording 收敛；真实 Codex 开发文档输出复核已推动默认模板轻量化，普通 PRD 轻量样例已通过。 | 后续观察真实输出，不新增 skill / harness。 |
...
| 18 个剩余临时 / 覆盖 proposal 是否进入 30 天删除候选 | 建议进入候选，不马上删 | 能减少 proposal 噪音，同时保留 2026-06-02 后二次审批。 |
| 是否创建 5 天停滞自动提醒 | 暂不创建 | 先用台账试运行更稳；直接自动化可能误报。 |
| 是否把线程台账转 stable 文档 | 暂不转 | 先观察 1-2 周，避免过早固化。 |
| `ai-intel/raw/*` 怎么处理 | 不提交 raw，列入 2026-06-02 后删除候选 | 保留 30 天复核能力，同时不污染仓库。 |
| `docs/archive/notes/*` 怎么处理 | `答辩.md` 随 `graduation-defense-agent` closeout；`Remod开发.md` 已列为 2026-06-03 后删除候选 | 避免 archive 策略被未审历史材料绑死。 |
...
| `memory-learning-extractor` 是否继续插件化 | 暂不插件化 | 已从 draft 收口为 candidate；继续插件化会增加维护成本。 |
...
4. 提交剩余 18 个未跟踪 proposal 的 30 天删除候选精确清单；删除必须等 2026-06-02 后二次批准。
5. `projects/_archives` 和项目 zip 统一归档策略。
6. 提交 `ai-intel/raw/*` 生命周期候选清单，不提交 raw 本体。
7. 输出本轮治理修复总验收报告。
8. 观察线程台账 1-2 周，再决定是否创建 5 天停滞提醒自动化。
