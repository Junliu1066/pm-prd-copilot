...
- 工程可用开发文档偏好：当用户说“开发文档”时，默认目标是工程可用的开发执行文档，而不是方向对齐文档。开发文档进入 `Ready / 开发执行版` 前，必须能回答四个问题：什么时候可以开发、AI 怎么可评估、异步任务怎么可靠、候选结果怎么安全发布。适用时必须包含 Development Ready Gate、AI Evaluation Gate、Async Reliability Gate、Candidate Publish Gate；未通过门禁的文档只能标记为 `Draft / 方向对齐版`。
...
- Engineering-usable development document preference: when the user says “开发文档”, the default target is an engineering-usable execution document, not an alignment document. Before a development document can be marked `Ready / 开发执行版`, it must answer four questions: when development can start, how AI can be evaluated, how async jobs stay reliable, and how candidate results can be safely published. When applicable, include the Development Ready Gate, AI Evaluation Gate, Async Reliability Gate, and Candidate Publish Gate. Documents that do not pass the gates may only be marked `Draft / 方向对齐版`.
