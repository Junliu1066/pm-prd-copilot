...
Use the upstream `skills/` directory's 20 core engineering skills as the routing source. `using-agent-skills` is a meta skill, not a development/testing task route for this agent.

Route every skill or harness decision through A2 `dynamic_skill_harness_plan`:
...
Record skip reasons for irrelevant skills or harnesses.
...
| Skill | Reason |
|---|---|
| `idea-refine` | concept discovery |
| `spec-driven-development` | PRD/spec authorship |
| `context-engineering` | session/context recovery, not the implementation task itself |
...
`planning-and-task-breakdown` may be used only to fill missing development/testing task packages without changing requirement scope.
